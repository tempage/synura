...
func (r *Runtime) newDOMNodeObject(node *html.Node) goja.Value {
	...
	r.defineDOMGetter(obj, "textContent", func() goja.Value { return r.vm.ToValue(textContent(node)) })
	r.defineDOMGetter(obj, "innerText", func() goja.Value { return r.vm.ToValue(textContent(node)) })
	r.defineDOMGetter(obj, "text", func() goja.Value { return r.vm.ToValue(textContent(node)) })
