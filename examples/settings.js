...
const initialFormFields = [
    { type: 'string', name: 'apiKey', label: 'API Key', value: '' },
    { type: 'number', name: 'limit', label: 'Item Limit', value: 20 },
...
const handler = {
    home: function () {
        ...
        if (result.success) {
            synura.connect(result.viewId, { from: "settings" }, (e) => handler.onViewEvent(e));
        }
    },
            ...
            switch (button) {
                case 'Save':
                    ...
                    synura.update(event.viewId, {
                        models: {
                            snackbar: { message: "Settings saved successfully!" }
                        }
                    });
                ...
                case 'Dialog':
                    ...
                    const currentFields = initialFormFields.map(field => {
                        if (event.data.hasOwnProperty(field.name)) {
                            return Object.assign({}, field, { value: event.data[field.name] });
                        }
                        return field;
                    });
                    ...
                    const dialogResult = synura.open('/dialogs/input', {
                        ...
                        models: {
                            body: { details: currentFields },
                            buttons: { details: ['OK', 'Cancel'] }
                        }
                    });

                    if (dialogResult.success) {
                        synura.connect(dialogResult.viewId, { from: "dialog" }, (dialogEvent) => {
                                ...
                                if (dialogEvent.data.button === 'OK') {
                                    ...
                                    const newFields = initialFormFields.map(field => {
                                        if (dialogEvent.data.hasOwnProperty(field.name)) {
                                            return Object.assign({}, field, { value: dialogEvent.data[field.name] });
                                        }
                                        return field;
                                    });
